...
function JSMap(mapElement)
{
	...
	console.log("Creating Leaflet JSMap...");
    /* custom fix for MSIE */
    ...
    this.leafletMap.setView(L.latLng(DEFAULT_CENTER.lat,DEFAULT_CENTER.lon),13);
	...
	//Here Map
	console.log("Initializing BingMap..");
	var here = new L.tileLayer.here({appId: HERE_MAPID, appCode: HERE_MAPCODE}); //("Anqm0F_JjIZvT0P3abS6KONpaBaKuTnITRrnYuiJCE0WOhH6ZbE4DzeT6brvKVR5");
	here.gtsMapName = "here";
	this.mapLayer.push(here);
		
	console.log("Initializing MapBox..");
	//https://www.mapbox.com/mapbox.js/example/v1.0.0/leaflet-studio-style/
	...
	var mapBoxAccessToken = MAPBOX_API; //'pk.eyJ1IjoicmFteWFzaXZoIiwiYSI6ImNqZWIzd3kxbjBiczMzM2xhanlubzBsbDYifQ.vYDOlHTViIY-OAiTNFd_1A';
	var mapBox = L.tileLayer(
		'https://api.mapbox.com/styles/v1/mapbox/emerald-v8/tiles/{z}/{x}/{y}?access_token='+mapBoxAccessToken, {
        tileSize: 512,
        zoomOffset: -1,
        attribution: '© <a href="https://www.mapbox.com/feedback/">Mapbox</a> © <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
		}
	);
	mapBox.gtsMapName = "mapbox";
	this.mapLayer.push(mapBox);
	
	//Bing Map
	console.log("Initializing BingMap..");
	var bing = new L.BingLayer(BING_MAP_API); //("Anqm0F_JjIZvT0P3abS6KONpaBaKuTnITRrnYuiJCE0WOhH6ZbE4DzeT6brvKVR5");
	bing.gtsMapName = "bing";
	this.mapLayer.push(bing);
	
	//Google Map
	console.log("Initializing GoogleBox..");
	//var map = new L.Map('map', {center: new L.LatLng(51.51, -0.11), zoom: 9});
    //var googleLayer = new L.Google('ROADMAP');
	var googleLayer = L.gridLayer.googleMutant({
			type:'roadmap'
		});
	googleLayer.gtsMapName = "google-roadmap";
	this.mapLayer.push(googleLayer);
	
	
    //var googleLayerSatellite = new L.Google('SATELLITE');
	var googleLayerSatellite = L.gridLayer.googleMutant({
			type:'satellite'
		});
	googleLayerSatellite.gtsMapName = "google-satellite";
	this.mapLayer.push(googleLayerSatellite);
	
    //var googleLayerHybrid = new L.Google('HYBRID');
	var googleLayerHybrid = L.gridLayer.googleMutant({
			type:'hybrid'
		});	
	googleLayerHybrid.gtsMapName = "google-hybrid";
	this.mapLayer.push(googleLayerHybrid);

    //var googleLayerTraffic = new L.Google('TRAFFIC');
	var googleLayerTraffic = L.gridLayer.googleMutant({
			type:'traffic'
		});	
	googleLayerTraffic.gtsMapName = "google-traffic";
	this.mapLayer.push(googleLayerTraffic);
	...
	*/
    ...
    if (this.mapLayer.length > 0) {
		console.log("Adding currentLayer to Map..");
		this.currentLayer = this.mapLayer[0];
        this.leafletMap.addLayer(this.currentLayer); // add first mapLayer
    ...
    try {
		...
		this.leafletMap.on("zoomend", function(pe) { // ZoomEnd
			console.log("HasMapPointLoaded is " + HasMapPointLoaded);
			
			if (HasMapPointLoaded == true) {
				console.log("in zoomend...");
				HasUserZoomed = true;
				UserZoomFactor = jsmap.leafletMap.getZoom();
				console.log("In zoomend...UserZoomFactor is " + UserZoomFactor);
			}
		});
		...
		if (jsvGeozoneMode == false) {
			this.leafletMap.on('contextmenu', function(ce) {
				jsmap._event_ContextMenu(ce);
				//alert("Right Click...");
				//console.log(e);
				//alert(e.latlng);
			});
	...
	console.log("Initializating completed...");
};
//GoTracky
JSMap.prototype.changeMapLayer = function(layerName) {
	if (this.currentLayer.gtsMapName == layerName) {
		//console.log("Same layer name...")
	} else {
        if (this.mapLayer.length > 1) {
			for (var i = 0; i < this.mapLayer.length; i++) {
				var mapLay  = this.mapLayer[i];
                var name = mapLay.gtsMapName;
                if (name == layerName) {
					console.log("Layer changed..")
					this.leafletMap.removeLayer(this.currentLayer); // Remove layer
					this.currentLayer = mapLay;
					this.leafletMap.addLayer(this.currentLayer); // add first mapLayer
					return;
				}
			}
...
JSMap.prototype._initOpenStreetMap = function(mapElement)
{
    ...
    OpenStreetMap_Mapnik.gtsMapName = "rede";
...
JSMap.prototype._clearMarkerLayer = function()
{
    if (this.markerLayer !== null) {
		console.log("in _clearMarkerLayer...this.markerLayer is not null...");
        /* clear Leaflet markers */
		try {
		    var markers = this.markerLayer.getLayers();
			if (markers !== null) {
				for (var i = 0; i < markers.length; i++) {
					//console.log("in _clearMarkerLayer " + i + " - " + markers[i]);
					try {
						markers[i].closePopup();
						markers[i].unbindPopup();
						this.markerLayer.removeLayer(markers[i]);
					} catch(err) {
						console.log("ERROR[_clearMarkerLayer removeLayer]: " + err);
					}
				}
			}
		} catch (e) {
...
JSMap.prototype._createRulerFeature = function(pt1, pt2)
{
    if ((pt1 !== null) && (pt2 !== null)) {
        var rp = [];
        rp.push(this._toLeafletPoint(pt1));
        rp.push(this._toLeafletPoint(pt2));
...
{
	//console.log("_drawFeatures..1111");
    ...
    if (this.drawFeatures && (this.drawFeatures.length > 0)) {
        for (var i = 0; i < this.drawFeatures.length; i++) {
			//console.log("in this.drawLayer[0]-222");
			//console.log(this.drawLayer[0]);
			//console.log(this.drawFeatures[i]);
			//this.markerLayer.addLayer(this.drawFeatures[i]);
            this.drawLayer[0].addLayer(this.drawFeatures[i]); // L.featureGroup
        }
    }
	//console.log(this.drawLayer[0]);
	try { 
		//console.log("re-adding this.drawLayer[0]");
        this.leafletMap.addLayer(this.drawLayer[0]);  // must re-add layer
    } catch (e) {
		console.log(e);
    }
...
{
    try {
        var opt = this._toLeafletPoint(center);
        if (!zoom || (zoom == 0)) {
...
JSMap.prototype.JSDrawPushpins = function(pushPins, recenterMode, replay)
{
	...
	console.log("recenterMode is " + recenterMode);
    if (recenterMode > 0) {
        var centerPt;
        var zoomFactor;
        if (pointCount <= 0) {
            centerPt   = this._toLeafletPoint(DEFAULT_CENTER);
            zoomFactor = DEFAULT_ZOOM;
        } else 
        if (recenterMode == RECENTER_LAST) { // center on last point
			console.log("in recenterMode RECENTER_LAST");
            var pp     = drawPushpins[drawPushpins.length - 1];
            centerPt   = this._toLeafletPoint(pp);
        ...
        try {
			console.log("zoomFactor1 is " + zoomFactor);
			//if (zoomFactor < DEFAULT_ZOOM) {
			//	zoomFactor = DEFAULT_ZOOM;
			//}
			if (HasUserZoomed == true) {
				zoomFactor = UserZoomFactor;
			}
			console.log("zoomFactor2 is " + zoomFactor);
...
JSMap.prototype._addPushpin = function(pp, layer)
{
    ...
    try {
        ...
        if (pp.iconUrl) {
			...
			if (pp.showDeviceLabel && pp.evRcd.device) {
				ppMarker.bindTooltip(pp.evRcd.device, 
				{
					permanent: true, 
					direction: 'top'
				});
        ...
        if (pp.getHTML()) {
            pp.popup = L.popup()
                .setLatLng(this._toLeafletPoint(pp))
...
JSMap.prototype._replayPushpins = function(replay)
{
	...
	if (this.lastReplayMarker) {
		if (this.lastReplayMarker.isRunning()) {
			//console.log("SlideTo is still running...");
	...
	if (this.replayIndex > 1) {
		lastPP = this.replayPushpins[this.replayIndex - 1]
	}
    while (true) {
        // -- end of replay
        ...
        if (!geoIsValid(pp.lat,pp.lon)) {
			...
			if (lastPP) {
				console.log("Current Lat/Long is " + pp.lat + "/" + pp.lon);
				console.log("Previosu Lat/Long is" + lastPP.lat + "/" + lastPP.lon);
				if (pp.lat == lastPP.lat && pp.lon == lastPP.lon) {
					console.log("Same as previous Lat/Long");
					this.replayIndex++;
					continue;
				}
			}
    ...
    /* add pushpin */
    try {
		//console.log("in _replayPushpins");
		...
		//GoTracky
		if (lastNdx >= 0) {
			var lastpp = this.replayPushpins[lastNdx];
			//console.log(pp);
			//console.log("lastIconIdx " + this.lastIconIdx);
			//console.log("From Lat/Lng is " + lastpp.lat + "/" + lastpp.lon + " To Lat/Lng is " + pp.lat + "/" + pp.lon);
			var angle = angleFromCoordinate(lastpp.lat, lastpp.lon, pp.lat, pp.lon);
			//console.log("Angle is " + angle);
			//alert("Angle2 is " + angle);
			if (angle == 0 && lastAngle != null) {
				//console.log("Angle is " + angle + " lastAngle is " + lastAngle);
				angle = lastAngle;
			} else {
				lastAngle = angle;
			}
			
			var newEvRcd = {heading: angle};
			//var ppObj = jsmGetPushPinIcon(this.lastIconIdx,lastpp.evRcd);
			var ppObj = jsmGetPushPinIcon(this.lastIconIdx,newEvRcd);
			//console.log("ppObj is - ");
			//console.log(ppObj);
			//console.log("pp is - ");
			//console.log(pp);
			...
			var ppMarker = null;
			if (this.lastReplayMarker) {
				ppMarker = this.lastReplayMarker;
				//ppMarker.options.icon.options.iconUrl=addBaseHrefToIconUrl(ppObj.iconURL);
				var iIcon = L.icon(
					{
						iconUrl: addBaseHrefToIconUrl(ppObj.iconURL),
						iconSize: [ ppObj.iconSize[0], ppObj.iconSize[1] ],
						iconAnchor: [ ppObj.iconHotspot[0], ppObj.iconHotspot[1] ],
						popupAnchor: [ 0, -ppObj.iconHotspot[1]+3 ]
					  //shadowURL: 
					  //shadowSize:
					  //shadowAnchor:
					}
				);
				ppMarker.setIcon(iIcon);
			} else {
				//console.log("ppObj.iconUrl is " + ppObj.iconUrl);
				var iIcon = L.icon(
					{
						iconUrl: addBaseHrefToIconUrl(ppObj.iconURL),
						iconSize: [ ppObj.iconSize[0], ppObj.iconSize[1] ],
				...
				if (ppObj.showDeviceLabel && pp.evRcd.device) {
					ppMarker.bindTooltip(pp.evRcd.device, 
					{
						permanent: true, 
						direction: 'top'
				...
				this.markerLayer.addLayer(ppMarker);
				this.lastReplayMarker = ppMarker;
			}
			//console.log("ppMarker is - ");
			//console.log(ppMarker);
			
			var keepAtCenter = false;
			console.log("keepAtCenter is "+ document.getElementById('ReplayMap').mapKeepAtCenter.checked);
			if (document.getElementById('ReplayMap').mapKeepAtCenter.checked) {
				console.log("keepAtCenter is true");
				keepAtCenter = true;
			}
			
			console.log("this.replayInterval is " + this.replayInterval);
			ppMarker.slideTo([pp.lat,pp.lon], {duration:this.replayInterval, keepAtCenter: keepAtCenter});
		}
		//GoTracky
        //this._addPushpin(pp, this.markerLayer);
        if (replay && (replay >= 2)) {
...
JSMap.prototype.JSDrawShape = function(type, radiusM, verticePts, color, zoomTo, desc, ppNdx)
{
    ...
    /* zoom bounds */
    var mapBounds = (zoomTo || (ppNdx >= 0))? L.latLngBounds() : null;
    ...
    if (didDrawShape && (ppNdx >= 0) && (ppNdx < jsvPushpinIcon.length) && mapBounds) {
        ...
        var popup = L.popup()
            .setLatLng(this._toLeafletPoint(ppLL))
...
*** @param center   The center point (L.latLng) of the circle
...
*** @param vertices   An array of polygon vertice points (L.latLng) of the circle
...
*** @param me  The MouseEvent
**/
JSMap.prototype._event_OnMouseUp = function(me) // MousrEvent
{
    var meXY     = me.layerPoint;    // L.point
    var meLL     = me.latlng         // L.latLng
    var domME    = me.originalEvent; // DOMMouseEvent
...
JSMap.prototype._event_OnMouseMove = function(me) // MouseEvent
{
    ...
    if (this.dragType == DRAG_GEOZONE_CENTER) {
        ...
        this.primaryCenter = this._toLeafletPoint(CC);
...
JSMap.prototype._event_ContextMenu = function(ce) // PopupEvent
{
	var self=this;
	//console.log(ce);
	var lat1 = ce.latlng.lat.toFixed(5);
    var lng1 = ce.latlng.lng.toFixed(5);
  
	// populate yor box/field with lat, lng
    //console.log("Lat=" + lat1 + "; Lng=" + lng1);

    var html = "<div id='jsmap_gz1' style='padding:0px; margin:0px; display:block;'>Create GeoZone</div>";
    html += "<div id='jsmap_gz2' style='padding:0px; margin:0px; display:block;'>";
    html += "<form id='jsmap_form_gz'>"
    html += "<table class='infoBoxTable' cellspacing='1' cellpadding='1' border='0'>";
    html += "<tr class='infoBoxRow'>"
    html += "<td class='infoBoxCell'><b>GeoZone Id:</b> <input type='text' id='z_newid' size=15 maxlength=20></td>"
    html += "</tr>";
    html += "<tr class='infoBoxRow'>"
    html += "<td class='infoBoxCell'><b>Location:</b>" + lat1 + "/" + lng1 + "</td>"
    html += "</tr>";
    html += "<tr class='infoBoxRow'>"
    html += "<td colspan='2' class='infoBoxCell'><b>GeoZone Name:</b> <input type='text' id='z_newname' size=15 maxlength=20></td>"
    html += "</tr>";
    html += "<tr class='infoBoxRow'>"
    html += "<td class='infoBoxCell'><b>Is Place Of Interest:</b> <input type='checkbox' id='z_ispoi' value='1'></td>"    
    html += "<td class='infoBoxCell'><input type='button' id='create' value='Create' onclick=\"javascript:jsmPushPinGeoZoneSubmitForm();\"></td>"	
    html += "</tr>";
    html += "<input type='hidden' id='z_lat0' value='"+ lat1 +"'></td></tr>";
    html += "<input type='hidden' id='z_lon0' value='" + lng1 + "'></td></tr>";
    html += "</table>";
    html += "</form>";
    html += "</div>";

	if (self.geozonePopup == undefined) {
		self.geozonePopup = L.popup();
	}
