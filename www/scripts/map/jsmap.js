...
var RECENTER_ALL                = 2; // center/zoom on all points
var RECENTER_PAN                = 3; // pan to last point
var RECENTER_ZOOM               = RECENTER_LAST; // normal center zoom on all points


/* Geozone types */
var ZONE_POINT_RADIUS           = 0;
var ZONE_BOUNDED_RECT           = 1; // not yet supported
var ZONE_SWEPT_POINT_RADIUS     = 2; 
var ZONE_POLYGON                = 3;
var ZONE_POINT_OF_INTEREST      = 4; 
...
// ----------------------------------------------------------------------------
// --- JSMapPushpin
function addBaseHrefToIconUrl(iconUrl) {
	if (iconUrl) {
		if (iconUrl.indexOf("Marker?") == -1) {			
			...
			if (location.protocol == "file:") {
				if ((typeof mobileBaseUrl != 'undefined') && (typeof mobilePath != 'undefined') ) {
					...
					alert("mobileBaseUrl or mobilePath is not configured!!");
				}			
			}
			//console.log(iUrl);
			return iUrl;			
    ...
    //shadowURL, shadowSize)
{
	...
	//console.log(ppIcon);
	//console.log(ppIcon.iconSize);
    /* icon attributes */
    this.iconUrl     = addBaseHrefToIconUrl(ppIcon.iconURL);     //*/ iconURL;
    this.iconSize    = ppIcon.iconSize;    //*/ iconSize;
    this.iconHotspot = ppIcon.iconHotspot; //*/ iconHotspot;
    this.shadowUrl   = addBaseHrefToIconUrl(ppIcon.shadowURL);   //*/ shadowURL;
    this.shadowSize  = ppIcon.shadowSize;  //*/ shadowSize;
	
	//GoTracky
	this.showDeviceLabel = ppIcon.showDeviceLabel;
    ...
    this.bgUrl       = hasBG? addBaseHrefToIconUrl(ppIcon.bgURL)    : null; // "extra/images/pp/CrosshairRed.gif";
...
JSMapPushpin.prototype.getHTML = function()
{
	...
	// GoTracky
    if (evRcd.batteryLevel &&  evRcd.batteryLevel != "") {
        h += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Battery Level:</b> "+evRcd.batteryLevel+"</td></tr>";
    }
	
    if (evRcd.ignition && evRcd.ignition != "" && evRcd.ignition != "-" ) {
        h += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Ignition:</b> "+ getOnOff(evRcd.ignition) +"</td></tr>";
    }
	
	if (evRcd.acOnOff && evRcd.acOnOff != "" && evRcd.acOnOff != "-" ) {
        h += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>AC:</b> "+ getOnOff(evRcd.acOnOff)+"</td></tr>";
    }	

	if (evRcd.doorOnOff && evRcd.doorOnOff != "" && evRcd.doorOnOff != "-" ) {
        h += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>AC:</b> "+ getOnOff(evRcd.doorOnOff)+"</td></tr>";
    }
	
	if (evRcd.fuel && evRcd.fuel != "") {
        h += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Fuel:</b> "+evRcd.fuel+"</td></tr>";
    }
	
	if (evRcd.temperature && evRcd.temperature != "") {
        h += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Temperature:</b> "+evRcd.temperature+"</td></tr>";
    }	
	//GoTracky
	...
	//GoTracky
	h += '<div id="jsmap_gz1" style="padding: 0px; margin: 0px; display: block; visibility: display;">';
	h += '<a onclick="javascript:jsmPushPinGeoZoneShowForm();">Create Geozone</a>';
	h += '</div>';

    h += "<div id='jsmap_gz2' style='padding:0px; margin:0px; display:none; visibility: hidden;'>";
    h += "<form id='jsmap_form_gz'>"
    h += "<table class='infoBoxTable' cellspacing='1' cellpadding='1' border='0'>";
    h += "<tr class='infoBoxRow'>"
    h += "<td class='infoBoxCell'><b>GeoZone Id:</b> <input type='text' id='z_newid' size=15 maxlength=20></td>"
    h += "</tr>";
    h += "<tr class='infoBoxRow'>"
    h += "<td class='infoBoxCell'><b>Location:</b>" + evRcd.latitude + "/" + evRcd.longitude + "</td>"
    h += "</tr>";
    h += "<tr class='infoBoxRow'>"
    h += "<td colspan='2' class='infoBoxCell'><b>GeoZone Name:</b> <input type='text' id='z_newname' size=15 maxlength=20></td>"
    h += "</tr>";
    h += "<tr class='infoBoxRow'>"
    h += "<td class='infoBoxCell'><b>Is Place Of Interest:</b> <input type='checkbox' id='z_ispoi' value='1'></td>"    
    h += "<td class='infoBoxCell'><input type='button' id='create' value='Create' onclick=\"javascript:jsmPushPinGeoZoneSubmitForm();\"></td>"	
    h += "</tr>";
    h += "<input type='hidden' id='z_lat0' value='"+  evRcd.latitude +"'></td></tr>";
    h += "<input type='hidden' id='z_lon0' value='" + evRcd.longitude + "'></td></tr>";
    h += "</table>";
    h += "</form>";
    h += "</div>";
...
// }
        ...
        for (var p = startNdx; p < pts.length; p++) {
            ...
            if (evRcd.validGPS) {
                ...
                if (evRcd.timestamp > 0) {
					...
					//GoTracky
					jsmap.lastIconIdx = evRcd.iconNdx;
					//GoTracky
	...
	if (!IS_FLEET) {
		//console.log("lastEvRcd is - ");
		//console.log(lastEvRcd);
		var scope = angular.element(document.getElementById("mainWrapperDiv")).scope();
		scope.loadDeviceInfo(lastEvRcd);
...
function jsmGetPushPinIcon(icon, e)
{
    ...
    if (pp.iconEval) {
        try {
			...
			console.log("iconEval Error" + err);
    ...
    if ((typeof pp.bgPP !== 'undefined') && (pp.bgPP >= 0) && (pp.bgPP < jsvPushpinIcon.length)) {
        ...
        if (bgPP.iconEval) {
            try {
				...
				console.log("background iconEval Error" + err);
                pp.bgURL = null;
            }
        }
    }
...
function jsmShowDetailReport() 
{
	...
	//alert("in jsmShowDetailReport");
	
	//alert(ID_DETAIL_TABLE);
	...
	*/
    ...
    html += "<table id='LocationDetailsReport' class='fixed_headers' cellspacing='0' cellpadding='0'>\n";
    html += "<thead class='fix_table_header_bg_color'>\n";
    if (jsvPartialData) {
    ...
    for (var i = 0; i < dpt.length; i++) {
        ...
        // address
        if (SHOW_ADDR) { 
            html += "<td class='"+dataClass+"'>" + pt.address + "&nbsp;</td>"; 
        }
        // optional fields
        if (SHOW_OPT_FIELDS) {
    ...
    try {
        ...
        if (btn) {
            if (state == REPLAY_RUNNING) {
				...
				btn.innerHTML="<i id='mapReplayButton' name='replayMap' class='material-icons'>pause</i>";
            } else if (state == REPLAY_PAUSED) {
                // Replay is paused, option is "Continue"
                //btn.src = "images/Continue20.png";
				btn.innerHTML="<i id='mapReplayButton' name='replayMap' class='material-icons'>play_arrow</i>";
            } else {
...
function jsmSetElementValue(item, value)
{
    if (item != null) {
        item.value = value;
		try {
			angular.element(item).triggerHandler('change');
		} catch(err) {
			console.log("Angular Trigger change error - " + err);
		}
    } else {
...
{
	console.log("jsmSetIDValue to " + idName);
...
function evHeadingURL(e,color) {
    if (e.speedKPH < 1.0) {
        // probably not moving
        return "images/pp/pin30_"+color+".png";
    } else {
        var x = Math.round(e.heading / 45.0) % 8;
        return "images/pp/pin30_"+color+"_h"+x+".png";
    }
}
...
//GoTracky
/**
*** Returns a 'orange' icon URL based on the event heading
*** @param e  The 'MapEventRecord' object
**/
function evHeadingOrangeURL(e)
{
    if (e.speedKPH < 1.0) {
        // probably not moving
        return "images/pp/pin30_orance_dot.png";
    } else {
        var x = Math.round(e.heading / 45.0) % 8;
        return "images/pp/pin30_orange_h"+x+".png";
    }
};

function evHeadingBlueURL(e)
{
    if (e.speedKPH < 1.0) {
        // probably not moving
        return "images/pp/pin30_blue_dot.png";
    } else {
        var x = Math.round(e.heading / 45.0) % 8;
        return "images/pp/pin30_blue_h"+x+".png";
    }
...
*** @param e  The 'MapEventRecord' object
**/
function evColorTrack(e, trackColor)
{
    if (e.speedKPH < 1.0) {
        // probably not moving
        return "images/pp/pin30_red_dot.png";
        ...
        var x = Math.round(e.heading / 45.0) % 8;
        return "images/pp/pin30_"+trackColor+"_h"+x+".png";
    }
};
function evVehicleRotate(e,vehicleIcon,degree) {
	//console.log("in evVehicleRotate - vehicleIcon is " + vehicleIcon + " - degree is " + degree);
	//console.log(e);
	var numberOfIcons = 360 / degree;
    var x = Math.round(e.heading / degree) % numberOfIcons;
	//console.log("numberOfIcons is " + numberOfIcons + " x is " + x);
    return "images/pp/vehicle/"+vehicleIcon+"_h"+x+".png";
};
//GoTracky
...
//GoTracky
/** 
*** Function to hide the Create GeoZone and Display the fields
***
**/
function jsmPushPinGeoZoneShowForm() 
{
	document.getElementById('jsmap_gz2').style.display='block';
	document.getElementById('jsmap_gz2').style.visibility='visible';
	document.getElementById('jsmap_gz1').style.visibility='hidden';
	document.getElementById('jsmap_gz1').style.display='none';
}

function jsmPushPinGeoZoneSubmitForm()
{
	//alert("jsmPushPinGeoZoneSubmitForm");
	var gzForm = document.getElementById('jsmap_form_gz');
	var geoZoneId = gzForm.z_newid.value;
	var geoZoneName = gzForm.z_newname.value;
	var geoIsPOI = "N";
	
	if (gzForm.z_ispoi.checked) {
		geoIsPOI = "Y";
	}

	var lat = gzForm.z_lat0.value;
	var lon = gzForm.z_lon0.value;

	if ( geoZoneId.trim().length == 0 )
	{
		alert("GeoZone Id should be entered");
		gzForm.z_newid.focus();
		return;
	}

	if ( geoZoneName.trim().length == 0 )
	{
		alert("GeoZone Name should be entered");
		gzForm.z_newname.focus();
		return;
	}

	var GEOZONE_CREATE_URL = API_URL + "/Track?page=zone.info&page_cmd=pushpincrt";
    	var url = GEOZONE_CREATE_URL + 
        	"&_uniq=" + Math.random() +  // necessary to make the URL unique
        	"&" + "z_newid" + "=" + strEncode(geoZoneId) +
        	"&" + "z_ispoi" + "=" + strEncode(geoIsPOI) +
        	"&" + "z_radius" + "= 100" +
        	"&" + "z_lat0" + "=" + strEncode(lat) +
        	"&" + "z_lon0" + "=" + strEncode(lon) +
        	"&" + "z_desc" + "=" + strEncode(geoZoneName);
    ...
    try {
		...
		.done(function( data ) {
			if (data.trim().length == 0) {
				document.getElementById('jsmap_gz1').innerHTML = "<b>Not Created!!</b>";
			} else if (data.trim().toUpperCase() == DATA_RESPONSE_LOGOUT) {
				alert(TEXT_TIMEOUT);
                document.getElementById('jsmap_gz1').innerHTML = "<b>" + TEXT_TIMEOUT + "</b>";
				//jsmSetLoadingPointsState(0);
            } else {
				...
				if (jsonData) {
					if (jsonData.Status == true) {
						document.getElementById('jsmap_gz1').innerHTML = "<b>" + jsonData.Message + "</b>";
						document.getElementById('jsmap_gz2').style.display='none';
						document.getElementById('jsmap_gz2').style.visibility='hidden';
						document.getElementById('jsmap_gz1').style.visibility='visible';
						document.getElementById('jsmap_gz1').style.display='block';
					} else {
        ...
        if (req) {
            ...
            req.onreadystatechange = function() {
                if (req.readyState == 4) {
                    ...
                    if (data.trim().toUpperCase() == DATA_RESPONSE_LOGOUT) {
						...
						//document.getElementById('jsmap_gz1').innerHTML = "<b>" + data.trim() + "</b>";
                    }
                } else
                if (req.readyState == 1) {
                    // alert('Loading points from URL: [' + req.readyState + ']\n' + mapURL);
            ...
            //jsmSetLoadingPointsState(1);
            document.getElementById('jsmap_gz1').innerHTML = "<b>Creating GeoZone....</b>";
			document.getElementById('jsmap_gz2').style.visibility='hidden';
			document.getElementById('jsmap_gz1').style.visibility='visible';
            req.send(null);
        } else {
            alert("Error [jsmGeoZoneCreate]:\n" + mapURL);
        }
		*/
    } catch (e) {
        alert("Error [jsmGeoZoneCreate]:\n" + e);
