...
function trackMapUpdateMap(limit, limitType, recenterMode, replay) 
{
    ...
    //try { document.getElementById(ID_CENTER_LAST_POINT_FORM).centerLastPoint.checked = false; } catch (e) {}
    //var dev = "87vt0032"; //GT IS_FLEET? document.SelectDeviceForm.group.value : document.SelectDeviceForm.device.value; // PARM_GEOUP/PARM_DEVICE
	var dev = $('#selectedDeviceId').val();
	//var dev = "87vt4763";
	//alert("dev selectd is " + dev);
    var rfr = mapCal_fr? mapCal_fr.getArgDateTime() : "";
    var rto = mapCal_to? mapCal_to.getArgDateTime() : "";
    var tmz = calGetTimeZone();
...
function trackMapUpdateKML() 
{
    if (KML_UPDATE_URL && (KML_UPDATE_URL != "")) {
        var dev = "863071014829169"; //GT IS_FLEET? document.SelectDeviceForm.group.value : document.SelectDeviceForm.device.value; // PARM_GEOUP/PARM_DEVICE
        var rfr = mapCal_fr? mapCal_fr.getArgDateTime() : "";
        var rto = mapCal_to.getArgDateTime();
        var tmz = calGetTimeZone();
